...
class GEPProblem():
    ...
    def __init__(self, T, N, G, L, pDemand, pGenAva, pVOLL, pWeight, pRamping, pInvCost, pVarCost, pUnitCap, pExpCap, pImpCap, sample_duration=12, shuffle=False, scale_input=True, train=0.8, valid=0.1, test=0.1):
        ...
        self.pExpCap_tensor = torch.tensor( # (L,) #! This is a mistake in Matthijs Arnoldus' thesis, the L is missing in math notation for impcap/expcap.
            [pExpCap[l] for l in self.L],
            device=self.DEVICE,
            dtype=self.DTYPE
        )

        self.pImpCap_tensor = torch.tensor( # (L,) #! This is a mistake in Matthijs Arnoldus' thesis, the L is missing in math notation for impcap/expcap.
        ...
        self.nineq = (len(self.G) * self.sample_duration + # 3.1b
        ...
        self.neq = len(self.N) * self.sample_duration # 3.1c
    ...
    @property
    def trainX_scaled(self):
        return self._trainX_scaled
    
    @property
    def validX_scaled(self):
        return self._validX_scaled
    
    @property
    def testX_scaled(self):
        return self._testX_scaled
    
    @property
    def train_rhs_eq(self):
        return self._train_rhs_eq
    
    @property
    def valid_rhs_eq(self):
        return self._valid_rhs_eq
    
    @property
    def test_rhs_eq(self):
        return self._test_rhs_eq
    
    @property
    def train_rhs_ineq(self):
        return self._train_rhs_ineq
    
    @property
    def valid_rhs_ineq(self):
        return self._valid_rhs_ineq
    
    @property
    def test_rhs_ineq(self):
        return self._test_rhs_ineq
    ...
    def targets_train(self):
        return self.train_extractor.opt_targets
    
    @property
    def targets_valid(self):
        return self.valid_extractor.opt_targets
    ...
    def _split_X_in_batches(self):
        ...
        if not hasattr(self, 'pDemand_scaler'):
            ...
            self.pGenAva_scaler = InputScaler(self.pGenAva_tensor)
        ...
        pGenAva_scaled_full = self.pGenAva_scaler.scale(self.pGenAva_tensor)  # Scale the entire pGenAva_tensor
        ...
        pGenAva_scaled_batched = pGenAva_scaled_full.view(len(self.G), B, self.sample_duration).permute(1, 0, 2).reshape(B, -1)  # [B, G*sample_duration]

        # Concatenate along the last dimension
        batched_unscaled_tensor = torch.cat((pDemand_batched, pGenAva_batched), dim=1)  # [B, N*sample_duration + G*sample_duration]
        batched_scaled_tensor = torch.cat((pDemand_scaled_batched, pGenAva_scaled_batched), dim=1)  # [B, N*sample_duration + G*sample_duration]

        return batched_unscaled_tensor, batched_scaled_tensor
    
    def _split_rhs_constants_in_batches(self):
        rhs_eqs = []
        rhs_ineqs = []

        for idx in range(len(self.time_ranges)):
            # Inequalities
            rhs_ineq = []
            # 3.1b
            rhs_ineq += [0] * len(self.G) * self.sample_duration
            # 3.1d
            rhs_ineq += self.pImpCap_tensor.unsqueeze(0).expand(self.sample_duration, 3).T.flatten() # L*T
            # 3.1e
            rhs_ineq += self.pExpCap_tensor.unsqueeze(0).expand(self.sample_duration, 3).T.flatten() # L*T
            # 3.1f
            rhs_ineq += [0] * len(self.G) * (self.sample_duration - 1)
            # 3.1g
            rhs_ineq += [0] * len(self.G) * (self.sample_duration - 1)
            # 3.1h
            rhs_ineq += [0] * len(self.G) * self.sample_duration
            # 3.1i
            rhs_ineq += [0] * len(self.N) * self.sample_duration
            #3.1j
            rhs_ineq += self.pDemand_tensor[:, self.sample_duration*idx:self.sample_duration*(idx+1)].flatten()
            #3.1k
            rhs_ineq += [0] * len(self.G)
            ...
            rhs_eq = []
            #3.1c
            rhs_eq += self.pDemand_tensor[:, self.sample_duration*idx:self.sample_duration*(idx+1)].flatten()
            rhs_eqs.append(rhs_eq)

        return torch.tensor(rhs_eqs), torch.tensor(rhs_ineqs)
    ...
    def _split_X_in_sets(self, train=0.8, valid=0.1, test=0.1):
        ...
        # Split the data
        trainX = self.X[train_indices]
        validX =self.X[valid_indices]
        testX = self.X[test_indices]
        trainX_scaled = self.X_scaled[train_indices]
        validX_scaled =self.X_scaled[valid_indices]
        testX_scaled = self.X_scaled[test_indices]
        train_rhs_eq = self.rhs_eq[train_indices]
        valid_rhs_eq = self.rhs_eq[valid_indices]
        test_rhs_eq = self.rhs_eq[test_indices]
        train_rhs_ineq = self.rhs_ineq[train_indices]
        valid_rhs_ineq = self.rhs_ineq[valid_indices]
        test_rhs_ineq = self.rhs_ineq[test_indices]
        ...
        self._validX = validX
        self._testX = testX
        self._trainX_scaled = trainX_scaled
        self._validX_scaled = validX_scaled
        self._testX_scaled = testX_scaled
        self._train_rhs_eq = train_rhs_eq
        self._valid_rhs_eq = valid_rhs_eq
        self._test_rhs_eq = test_rhs_eq
        self._train_rhs_ineq = train_rhs_ineq
        self._valid_rhs_ineq = valid_rhs_ineq
        self._test_rhs_ineq = test_rhs_ineq
    ...
    def dual_obj_fn(self, mu, lamb, rhs_eq, rhs_ineq):
        """ Objective function of the dual problem.
        max -mu^T d - lamb^T b
        
        However, PDL/ALM learn lambda with the sign flipped, so we flip the sign of lambda.
    ...
    def _e_lineflow_tensor(self, vLineFlow):
        ...
        ret_lb =  -1 * pImpCap_tensor - vLineFlow   # [B, L, T]
    ...
    #     # p_{gt} - p_{g,t-1} >= -R * UCAP_g * ui_g
    #     # -R * UCAP_g * ui_g <= p_{gt} - p_{g,t-1}
    #     # -R * UCAP_g * ui_g - (p_{gt} - p_{g,t-1}) <= 0
    ...
    def _e_ramping_down_tensor(self, vGenProd, vGenInv):
        ...
        # production_diff = vGenProd[:, :, :-1] - vGenProd[:, :, 1:] # [B, G, T-1]
        production_diff = vGenProd[:, :, 1:] - vGenProd[:, :, :-1] # [B, G, T-1]
        ramping_allowed = -1 * self.pRamping_tensor * self.pUnitCap_tensor.unsqueeze(0) * vGenInv # [B, G]
        ramping_allowed = ramping_allowed.unsqueeze(-1).expand(-1, -1, production_diff.shape[2]) # [B, G, T-1]
        # ret = production_diff - ramping_allowed # [B, G, T-1]
        ret = ramping_allowed - production_diff
        flattened = ret.reshape(vGenProd.shape[0], -1) # [B, G * (T-1)]
    ...
    def construct_eq_constraints(self, sample_idx):
        # Get masks
        gen_mask = self.gen_mask  # [N, G, T]
        inflow_mask = self.inflow_mask  # [N, L, T]
        outflow_mask = self.outflow_mask  # [N, L, T]

        # Build A matrix
        A_gen = gen_mask.flatten(start_dim=1)  # [N, G*T]
        A_inflow = inflow_mask.flatten(start_dim=1)  # [N, L*T]
        A_outflow = -outflow_mask.flatten(start_dim=1)  # [N, L*T]
        A_lossload = torch.eye(len(self.N) * self.sample_duration, device=self.DEVICE)  # [N*T, N*T]

        A = torch.cat((A_gen, A_inflow, A_outflow, A_lossload), dim=1)  # [N*T, num_variables]

        # Build x vector
        pDemand = self.pDemand_tensor[:, sample_idx * self.sample_duration:(sample_idx + 1) * self.sample_duration]
        x = pDemand.flatten()  # [N*T]

        return A, x

    def construct_ineq_constraints(self, sample_idx):
        # Get variables
        pGenAva = self.pGenAva_tensor[:, sample_idx * self.sample_duration:(sample_idx + 1) * self.sample_duration]
        pDemand = self.pDemand_tensor[:, sample_idx * self.sample_duration:(sample_idx + 1) * self.sample_duration]

        # Build G matrix (row by row for each constraint)
        G = []
        h = []

        # 3.1b: Production limit
        G_prod = torch.eye(self.vGenProd_size, device=self.DEVICE)  # [G*T, G*T]
        h_prod = (pGenAva * self.pUnitCap_tensor.unsqueeze(-1)).flatten()  # [G*T]
        G.append(G_prod)
        h.append(h_prod)

        # 3.1d, 3.1e: Line flow bounds
        G_line_lb = -torch.eye(self.vLineFlow_size, device=self.DEVICE)  # [L*T, L*T]
        G_line_ub = torch.eye(self.vLineFlow_size, device=self.DEVICE)  # [L*T, L*T]
        h_line_lb = -self.pImpCap_tensor.repeat(self.sample_duration)  # [L*T]
        h_line_ub = self.pExpCap_tensor.repeat(self.sample_duration)  # [L*T]
        G.append(G_line_lb)
        G.append(G_line_ub)
        h.append(h_line_lb)
        h.append(h_line_ub)

        # 3.1h: Positive generation
        G_pos_gen = -torch.eye(self.vGenProd_size, device=self.DEVICE)  # [G*T, G*T]
        h_pos_gen = torch.zeros(self.vGenProd_size, device=self.DEVICE)  # [G*T]
        G.append(G_pos_gen)
        h.append(h_pos_gen)

        # 3.1j: Missed demand <= demand
        G_missed_demand = torch.eye(self.vLossLoad_size, device=self.DEVICE)  # [N*T, N*T]
        h_missed_demand = pDemand.flatten()  # [N*T]
        G.append(G_missed_demand)
        h.append(h_missed_demand)

        # Combine all
        G = torch.cat(G, dim=0)  # [nineq, num_variables]
        h = torch.cat(h)  # [nineq]
