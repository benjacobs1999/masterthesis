...
class SimpleProblem:
    ...
    def __init__(self, Q, p, A, G, h, X, valid_frac=0.0833, test_frac=0.0833, modified_problem=False):
        self.modified = modified_problem
        ...
        # For altered, 'modified' problem, the constraint matrices A and G change per sample.
        if self.modified:
            # First 10 rows for G
            custom_G = self._G[:, :10, :]
            # Flatten for each sample
            custom_G = custom_G.flatten(start_dim=1)
            self._custom_X = torch.concat([self._X, self.h, custom_G], dim=1)
            
            # G = 3d
            self.num_ineq_constraints = self._G.shape[1]
            # A = 2d
            self.num_eq_constraints = self._A.shape[0]
        else:
            # G = 2d
            self.num_ineq_constraints = self._G.shape[0]
            # A = 2d
            self.num_eq_constraints = self._A.shape[0]

        self._Y = None
        if self.modified:
            self._xdim = self._custom_X.shape[1]
        else:
        ...
        # ! Code in if else is not used by me.
        if self.modified:
            pass
        else:
            det = 0
    ...
    def trainX(self):
        ...
        # return self.custom_X[: int(self.num * self.train_frac)]
    ...
    def obj_fn(self, Y):
        return self._original_obj_fn(Y)
    
    def _original_obj_fn(self, Y):
        return (0.5 * (Y @ self.Q) * Y +  self.p * Y).sum(dim=1)
    
    def _modified_obj_fn(self, Y):
        # return (self.p * Y).sum(dim=1)
        return (0.5 * (Y @ self.Q) * Y +  self.p * Y).sum(dim=1)
    # def obj_fn(self, Y):
        # return 1e8 * (0.5 * (Y @ self.Q) * Y + self.p * Y).sum(dim=1)
        # return (0.5 * (Y @ self.Q) + self.p * Y).sum(dim=1)

    def eq_resid(self, X, Y):
        return self._original_eq_resid(X, Y)
    
    def _original_eq_resid(self, X, Y):
        return X - Y @ self.A.T
    
    # ! For altered 'modifieder' problem.
    def _modified_eq_resid(self, X, Y):
        # X is the first 50, when using 50 eq constraints.
        custom_X = X[:, :self.num_eq_constraints].detach().cpu()
        return custom_X - Y @ self.A.T

    def ineq_resid(self, X, Y):
        return self._original_ineq_resid(X, Y)
    
    def _original_ineq_resid(self, X, Y):
        return Y @ self.G.T - self.h
    
    # ! For altered 'modifieder' problem.
    def _modified_ineq_resid(self, X, Y):
        # Extracting H from X (second 50 columns, assuming 50 inequality constraints)
        h = X[:, self.num_eq_constraints:self.num_eq_constraints+self.num_ineq_constraints].detach().cpu()

        # Extracting custom_G from X (all columns after X, H, A)
        custom_G = X[:, self.num_eq_constraints+self.num_ineq_constraints:].detach().cpu()

        # Reshape custom_G to match the first 10 rows of G
        custom_G = custom_G.reshape(X.shape[0], 10, self._ydim)  # Reshape for the batch size

        # Take only the first sample of G and clone it for modification
        G_first = self._G[0].clone()  # Shape is (M, P)

        # Repeat G_first for the batch size to avoid memory overlap
        G_first = G_first.unsqueeze(0).repeat(X.shape[0], 1, 1)  # Shape is (batch_size, M, P)

        # Assign custom_G to the first 10 rows of G
        G_first[:, :10, :] = custom_G  # Ensure dimensions match

        # resid = Y @ G_first.transpose(1, 2) - h
        residual = torch.bmm(Y.unsqueeze(1), G_first.transpose(1, 2)).squeeze(1) - h
    ...
    def custom_opt_solve(self, X, solver_type="osqp", tol=1e-4):
        if solver_type == "qpth":
            print("running qpth")
            start_time = time.time()
            res = QPFunction(eps=tol, verbose=False)(
                self.Q, self.p, self.G, self.h, self.A, X
            )
            end_time = time.time()
            ...
            for i, Xi in enumerate(X_np):
                ...
                G = self.G[i]
                my_A = np.vstack([A, G])
                my_l = np.hstack([Xi[:self.num_eq_constraints], -np.ones(H[i].shape[0]) * np.inf])
                my_u = np.hstack([Xi[:self.num_eq_constraints], H[i]])
                solver.setup(
                    P=csc_matrix(Q),
                    q=p,
                    A=csc_matrix(my_A),
    ...
    def calc_Y(self):
        if self.modified:
            return self._custom_calc_Y()
        else:
            return self._original_calc_Y()

    def _original_calc_Y(self):
    ...
    def _custom_calc_Y(self):
        Y = self.custom_opt_solve(self.custom_X)[0]
        feas_mask = ~np.isnan(Y).all(axis=1)
        self._num = feas_mask.sum()
        self._X = self._X[feas_mask]
        self._Y = torch.tensor(Y[feas_mask])
...
class ACOPFProblem:
    ...
    def __init__(self, filename, valid_frac=0.0833, test_frac=0.0833):
        ...
        self.quad_costs = torch.tensor(
            ppc["gencost"][:, 4], dtype=torch.get_default_dtype()
        )
        self.lin_costs = torch.tensor(
            ppc["gencost"][:, 5], dtype=torch.get_default_dtype()
        )
        self.const_cost = ppc["gencost"][:, 6].sum()

        self.pmax = torch.tensor(
            ppc["gen"][:, idx_gen.PMAX] / self.genbase, dtype=torch.get_default_dtype()
        )
        self.pmin = torch.tensor(
            ppc["gen"][:, idx_gen.PMIN] / self.genbase, dtype=torch.get_default_dtype()
        )
        self.qmax = torch.tensor(
            ppc["gen"][:, idx_gen.QMAX] / self.genbase, dtype=torch.get_default_dtype()
        )
        self.qmin = torch.tensor(
            ppc["gen"][:, idx_gen.QMIN] / self.genbase, dtype=torch.get_default_dtype()
        )
        self.vmax = torch.tensor(
            ppc["bus"][:, idx_bus.VMAX], dtype=torch.get_default_dtype()
        )
        self.vmin = torch.tensor(
            ppc["bus"][:, idx_bus.VMIN], dtype=torch.get_default_dtype()
        )
        ...
        Y = np.concatenate(
            [np.real(gen), np.imag(gen), np.abs(voltage), np.angle(voltage)], axis=1
        )
        ...
        self._partial_unknown_vars = np.concatenate(
    ...
    def obj_fn(self, Y):
        ...
        cost = (
            (self.quad_costs * pg_mw**2).sum(axis=1)
    ...
    def ineq_partial_grad(self, X, Y):
        eq_jac = self.eq_jac(Y)
        dynz_dz = -torch.inverse(eq_jac[:, :, self.other_vars]).bmm(
            eq_jac[:, :, self.partial_vars]
        )

        direct_grad = self.ineq_grad(X, Y)
        indirect_partial_grad = (
            dynz_dz.transpose(1, 2)
            .bmm(direct_grad[:, self.other_vars].unsqueeze(-1))
            .squeeze(-1)
        )
        ...
        full_grad[:, self.other_vars] = dynz_dz.bmm(
            full_partial_grad.unsqueeze(-1)
        ).squeeze(-1)
    ...
    def eq_jac(self, Y):
        ...
        Ydiagv = (
            lambda Y, v: Y.unsqueeze(0)
            .expand(v.shape[0], *Y.shape)
            .bmm(torch.diag_embed(v))
        )
        ...
        dreal_dvm = (
            -mdiag(cosva, YrvrYivi)
            - dtm(vr, Ydiagv(Yr, cosva) - Ydiagv(Yi, sinva))
            - mdiag(sinva, YivrYrvi)
            - dtm(vi, Ydiagv(Yi, cosva) + Ydiagv(Yr, sinva))
        )
        dreal_dva = (
            -mdiag(-vi, YrvrYivi)
            - dtm(vr, Ydiagv(Yr, -vi) - Ydiagv(Yi, vr))
            - mdiag(vr, YivrYrvi)
            - dtm(vi, Ydiagv(Yi, -vi) + Ydiagv(Yr, vr))
        )
        ...
        dreact_dvm = (
            mdiag(cosva, YivrYrvi)
            + dtm(vr, Ydiagv(Yi, cosva) + Ydiagv(Yr, sinva))
            - mdiag(sinva, YrvrYivi)
            - dtm(vi, Ydiagv(Yr, cosva) - Ydiagv(Yi, sinva))
        )
        dreact_dva = (
            mdiag(-vi, YivrYrvi)
            + dtm(vr, Ydiagv(Yi, -vi) + Ydiagv(Yr, vr))
            - mdiag(vr, YrvrYivi)
            - dtm(vi, Ydiagv(Yr, -vi) - Ydiagv(Yi, vr))
        )
        ...
        jac = torch.cat(
            [
                torch.cat(
                    [
                        dreal_dpg.unsqueeze(0).expand(vr.shape[0], *dreal_dpg.shape),
                        torch.zeros(
                            vr.shape[0], self.nbus, self.ng, device=self.device
                        ),
                        dreal_dvm,
                        dreal_dva,
                    ],
    ...
    def process_output(self, X, out):
        ...
        pg = (
            out2[:, : self.qg_start_yidx] * self.pmax
            + (1 - out2[:, : self.qg_start_yidx]) * self.pmin
        )
        qg = (
            out2[:, self.qg_start_yidx : self.vm_start_yidx] * self.qmax
            + (1 - out2[:, self.qg_start_yidx : self.vm_start_yidx]) * self.qmin
        )
        vm = (
            out2[:, self.vm_start_yidx :] * self.vmax
            + (1 - out2[:, self.vm_start_yidx :]) * self.vmin
        )

        va = torch.zeros(X.shape[0], self.nbus, device=self.device)
        va[:, self.nonslack_idxes] = out[:, self.va_start_yidx :]
        va[:, self.slack] = (
            torch.tensor(self.slack_va, device=self.device)
            .unsqueeze(0)
            .expand(X.shape[0], self.nslack)
        )
    ...
    def complete_partial(self, X, Z):
        ...
        Y_partial[:, self.pg_pv_zidx] = (
            Z[:, self.pg_pv_zidx] * self.pmax[1:]
            + (1 - Z[:, self.pg_pv_zidx]) * self.pmin[1:]
        )

        # Re-scale real parts of voltages
        Y_partial[:, self.vm_spv_zidx] = (
            Z[:, self.vm_spv_zidx] * self.vmax[self.spv]
    ...
    def opt_solve(self, X, solver_type="pypower", tol=1e-4):
        ...
        ppopt = ppoption.ppoption(
            OPF_ALG=560, VERBOSE=0, OPF_VIOLATION=tol
        )  # MIPS PDIPM
...
def PFFunction(data, tol=1e-5, bsz=200, max_iters=50):
    class PFFunctionFn(Function):
        ...
        def forward(ctx, X, Z):
            ...
            Y[:, data.qg_start_yidx : data.qg_start_yidx + data.ng] = -data.eq_resid(
                X, Y
            )[:, data.qflow_start_eqidx + data.spv]
            # solve for pg at slack bus (note: requires slack pg in Y to equal 0 at start of computation)
            Y[:, data.pg_start_yidx + data.slack_] = -data.eq_resid(X, Y)[
                :, data.pflow_start_eqidx + data.slack
            ]

            ctx.data = data
            ctx.save_for_backward(
                torch.cat(jacs),
                torch.cat(newton_jacs_inv),
        ...
        def backward(ctx, dl_dy):
            ...
            last_eqs = np.concatenate(
                [data.pflow_start_eqidx + data.slack, data.qflow_start_eqidx + data.spv]
            )
            ...
            jac3 = jac[:, last_eqs, :]
            dl_dvmva_3 = (
                -jac3[:, :, data.vm_start_yidx :]
                .transpose(1, 2)
                .bmm(dl_dy[:, last_vars].unsqueeze(-1))
                .squeeze(-1)
            )
            ...
            dl_dy_total = (
                dl_dy_3 + dl_dy
            )  # Backward pass vector including result of last step

            # Use precomputed inverse jacobian
            jac2 = jac[:, keep_constr, :]
            d_int = (
                newton_jac_inv.transpose(1, 2)
                .bmm(dl_dy_total[:, newton_guess_inds].unsqueeze(-1))
                .squeeze(-1)
            )
            ...
            dl_dz_2[:, data.vm_spv_zidx] = (
                -jac2[:, :, data.vm_start_yidx + data.spv]
                .transpose(1, 2)
                .bmm(d_int.unsqueeze(-1))
                .squeeze(-1)
            )

            dl_dx_2 = torch.zeros(dl_dy.shape[0], data.xdim, device=DEVICE)
            dl_dx_2[:, data.pv] = d_int[:, : data.npv]  # dl_dpd at pv buses
            dl_dx_2[:, data.pq] = d_int[
                :, data.npv : data.npv + len(data.pq)
            ]  # dl_dpd at pq buses
            dl_dx_2[:, data.nbus + data.pq] = d_int[
                :, -len(data.pq) :
            ]  # dl_dqd at pq buses
