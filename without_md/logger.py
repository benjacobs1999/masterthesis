...
class TensorBoardLogger():
    ...
    def log_train(self, data, primal_net, dual_net, rho, step):
        # TODO: Compute targets only once during initialization
        with torch.no_grad():
            Y = primal_net(self.X_train, self.eq_rhs_train, self.ineq_rhs_train)
            mu, lamb = dual_net(self.X_train, self.eq_cm_train)
            obj = data.obj_fn(Y, self.eq_cm_train, self.eq_rhs_train) # Containes penalization of negative missed demand
            ...
            if self.opt_targets:
                obj_target = data.obj_fn_log(self.Y_target_train, self.eq_cm_train, self.eq_rhs_train)
                dual_obj_target = data.dual_obj_fn_log(self.eq_rhs_train, self.ineq_rhs_train, self.mu_target_train, self.lamb_target_train)
            ...
            # print(self.eq_cm_train[0])
            # print(dual_eq_resid)
            # print(dual_ineq_resid.clamp(min=0))
            ...
            p_gt, f_lt = data.split_dec_vars_from_Y(Y)
            ...
            if self.opt_targets:
                ...
                p_gt_target, f_lt_target = data.split_dec_vars_from_Y(self.Y_target_train[:, :data.n_var_per_t], log=True)
            ...
            # h, b, d, e, = data.split_ineq_constraints(ineq_dist)
    ...
    def log_val(self, data, primal_net, dual_net, step):
        with torch.no_grad():
            ...
            obj = data.obj_fn(Y, self.eq_cm_valid, self.eq_rhs_valid) # Containes penalization of negative missed demand
            obj_train = data.obj_fn_train(Y) # Does not penalize negative missed demand
            ...
            if self.opt_targets:
                ...
                obj_target = data.obj_fn_log(self.Y_target_valid, self.eq_cm_valid, self.eq_rhs_valid)
                dual_obj_target = data.dual_obj_fn_log(self.eq_rhs_valid, self.ineq_rhs_valid, self.mu_target_valid, self.lamb_target_valid)
