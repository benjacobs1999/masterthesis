...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="simple",
        choices=["simple", "nonconvex", "acopf57"],
        help="problem type",
    )
    parser.add_argument(
        "--simpleVar", type=int, help="number of decision vars for simple problem"
    )
    parser.add_argument(
        "--simpleIneq",
        type=int,
    ...
    parser.add_argument(
        "--simpleEq", type=int, help="number of equality constraints for simple problem"
    )
    parser.add_argument(
        "--simpleEx", type=int, help="total number of datapoints for simple problem"
    )
    parser.add_argument(
        "--nonconvexVar", type=int, help="number of decision vars for nonconvex problem"
    )
    parser.add_argument(
        "--nonconvexIneq",
        type=int,
    ...
    for solver in solvers:
        ...
        Yvalid_opt, valid_time_total, valid_time_parallel = data.opt_solve(
            data.validX, solver_type=solver, tol=args["corrEps"]
        )
        Ytest_opt, test_time_total, test_time_parallel = data.opt_solve(
            data.testX, solver_type=solver, tol=args["corrEps"]
        )
