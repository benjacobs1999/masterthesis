...
class SimpleProblem:
    ...
    def __init__(self, Q, p, A, G, h, X, valid_frac=0.0833, test_frac=0.0833):
        ...
        self._p = torch.tensor(p, dtype=torch.float32, device=DEVICE)
        self._A = torch.tensor(A, dtype=torch.float32, device=DEVICE)
        self._G = torch.tensor(G, dtype=torch.float32, device=DEVICE)
        self._h = torch.tensor(h, dtype=torch.float32, device=DEVICE)
        self._X = torch.tensor(X, dtype=torch.float32, device=DEVICE)
        self._Y = None
    ...
    def L_rho(self, Yi, Xi, m, l, rho):
        ...
        return (
            ...
            * (
                torch.linalg.norm(
                    torch.maximum(ineq_resid, torch.zeros_like(ineq_resid)), 2
                )
                ** 2
                + torch.linalg.norm(eq_resid, 2) ** 2
            )
        )

    def ALM_solve(self, X, tol=1e-4):
        # print(f"self.Y_np.shape: {self.Y_np.shape}")
        # print(f"self.Q_np.shape: {self.Q_np.shape}")
        # print(f"self.p_np.shape: {self.p_np.shape}")
        # print(f"self.A_np.shape: {self.A_np.shape}")
        # print(f"self.G_np.shape: {self.G_np.shape}")
        # print(f"self.X_np.shape: {self.X_np.shape}")
        # print(f"self.h_np.shape: {self.h_np.shape}")

        # X_np = X.detach().cpu().numpy() if hasattr(X, "detach") else X

        self._Q = self._Q.to(DEVICE, dtype=torch.float32)
        self._p = self._p.to(DEVICE, dtype=torch.float32)
        self._A = self._A.to(DEVICE, dtype=torch.float32)
        self._G = self._G.to(DEVICE, dtype=torch.float32)
        self._h = self._h.to(DEVICE, dtype=torch.float32)
        self._X = self._X.to(DEVICE, dtype=torch.float32)

        X = X.to(DEVICE, dtype=torch.float32)

        mu_k = torch.zeros(self.G.shape[0], device=DEVICE, dtype=torch.float32)  # (50,)
        lamb_k = torch.zeros(
            self.A.shape[0], device=DEVICE, dtype=torch.float32
        )  # (50,)
        rho = 1.0
        tau = 0.5
        alpha = 10
        ...
        for i in range(len(X)):
            ...
            Yi = torch.nn.Parameter(
                torch.rand(100, device=DEVICE, dtype=torch.float32) * 2 - 1
            )

            for k in range(K):
                # Minimize L_rho for current `mu_k` and `lambda_k`
                optimizer = torch.optim.LBFGS(
                    [Yi], lr=1, max_iter=20, tolerance_grad=tol
                )

                def closure():
                    optimizer.zero_grad()  # Reset gradients
                    L_val = self.L_rho(Yi, Xi, mu_k, lamb_k, rho)
                    L_val.backward()  # Compute gradients
                    return L_val

                # Perform optimization step
                optimizer.step(closure)

                # Get current solution and residuals
                y_k = Yi.detach().clone()

                # Update dual variables using y_k
                mu_k = torch.maximum(
                    mu_k + rho * self.ineq_resid_ALM(y_k, Xi), torch.zeros_like(mu_k)
                )
                lamb_k = lamb_k + rho * self.eq_resid_ALM(y_k, Xi)

                # Calculate v_k to check for convergence
                inf_norm_eq_resid = torch.max(torch.abs(self.eq_resid_ALM(y_k, Xi)))
                sigma = torch.maximum(self.ineq_resid_ALM(y_k, Xi), -lamb_k / rho)
                inf_norm_sigma = torch.max(torch.abs(sigma))
        ...
        sols = torch.stack(Y).cpu().numpy()  # Final solutions for all instances

        return sols, total_time, parallel_time

    # ! Old implementation with scipy optimizer
    # def ALM_solve(self, X, tol=1e-4):
    #     # print(f"self.Y_np.shape: {self.Y_np.shape}")
    #     # print(f"self.Q_np.shape: {self.Q_np.shape}")
    #     # print(f"self.p_np.shape: {self.p_np.shape}")
    ...
    #     mu_k = np.zeros(self.G.shape[0])  # (50,)
    #     lamb_k = np.zeros(self.A.shape[0])  # (50,)
    #     rho = 1.0
    #     tau = 0.5
    #     alpha = 10
    #     K = 20
    ...
    #         Xi = X[i]  # Shape (50,)
    #         # The initial primal point is randomly sampled from the uniform distribution over [−1, 1].
    #         Yi = torch.rand(100, device=DEVICE, dtype=torch.float32) * 2 - 1
